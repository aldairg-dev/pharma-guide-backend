...
import {
  DosageResponse,
  FormattedDosageResponse,
  DosageData,
} from "../../types/dosage.types";
...
export class DosageModel extends DrugModel {
  ...
  static processData(data: any): DosageData {
    const dosageData: DosageData = {
      ...
      ajustes_funcionales: data.ajustes_funcionales || {
        renal: {},
        hepatica: {},
      },
      dosis_maxima_diaria: data.dosis_maxima_diaria || undefined,
      contraindicaciones: Array.isArray(data.contraindicaciones)
        ? data.contraindicaciones
        : [],
      interacciones_relevantes: Array.isArray(data.interacciones_relevantes)
        ? data.interacciones_relevantes
        : [],
    };
  ...
  static async getFormattedDosage(
    drugInfo: DrugInfo
  ...
  private static formatDosageText(dosageData: DosageData): string {
    ...
    if (dosageData.poblaciones_especiales) {
      ...
      const { pediatrica, geriatrica, embarazo_lactancia } =
        dosageData.poblaciones_especiales;
      if (pediatrica) formatted += `**Pediátrica:** ${pediatrica}\n\n`;
      if (geriatrica) formatted += `**Geriátrica:** ${geriatrica}\n\n`;
      if (embarazo_lactancia)
        formatted += `**Embarazo y Lactancia:** ${embarazo_lactancia}\n\n`;
    }
    ...
    if (dosageData.ajustes_funcionales) {
      ...
      if (renal && Object.values(renal).some((v) => v)) {
        ...
        if (renal.moderada)
          formatted += `- Insuficiencia moderada: ${renal.moderada}\n`;
        if (renal.severa)
          formatted += `- Insuficiencia severa: ${renal.severa}\n`;
      ...
      if (hepatica && Object.values(hepatica).some((v) => v)) {
        ...
        if (hepatica.child_pugh_A)
          formatted += `- Child-Pugh A: ${hepatica.child_pugh_A}\n`;
        if (hepatica.child_pugh_B)
          formatted += `- Child-Pugh B: ${hepatica.child_pugh_B}\n`;
        if (hepatica.child_pugh_C)
          formatted += `- Child-Pugh C: ${hepatica.child_pugh_C}\n`;
        formatted += "\n";
      }
    }
